...
const (
	DEFAULT_DEBUG                       = false
	DEFAULT_CONSUMER_TASK_QUANT_SECONDS = 60 * 60 * 200
...
func populateMarkets(cfg *Config) {
	...
	info, err := restClient.V5().Market().GetInstrumentsInfo(bybit.V5GetInstrumentsInfoParam{
		Category: "linear",
	})
	...
	for _, s := range info.Result.LinearInverse.List {
		if !strings.ContainsAny(string(s.Symbol), "_") && !strings.ContainsAny(string(s.Symbol), "-") && string(s.Symbol)[len(string(s.Symbol))-4:] != "PERP" {
			cfg.Consumer.Markets = append(cfg.Consumer.Markets, string(s.Symbol))
		}
	}
}

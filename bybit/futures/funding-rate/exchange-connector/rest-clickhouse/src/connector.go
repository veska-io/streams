...
import (
	...
	"github.com/veska-io/streams-connectors/bybit/futures/funding-rate/exchange-connector/rest-clickhouse/src/consumer"
...
func (c *Connector) Run() {
	...
	for response := range c.consumer.ResponseChan {
		...
		fRates, ok := response.Data.(*bybit.V5GetFundingRateHistoryResponse)
		if !ok {
			c.logger.Error("failed to cast response data to FundingRateHistory")
			continue
		}

		for _, fRate := range fRates.Result.List {
			timestamp, err := strconv.ParseInt(fRate.FundingRateTimestamp, 10, 64)
			if err != nil {
				c.logger.Error("failed to parse funding rate timestamp", slog.String("err", err.Error()))
				continue
			}
			...
			data := []any{
				...
				uint64(time.UnixMilli(timestamp).Truncate(time.Hour).UnixMilli()),
				fRate.FundingRate,

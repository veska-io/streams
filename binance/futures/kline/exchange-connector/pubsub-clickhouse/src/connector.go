...
func (c *Connector) Run() {
	...
	for msg := range c.consumer.DataStream {
		...
		if err := proto.Unmarshal(msg.Data, kline); err != nil {
		...
		data := []any{
			...
			kline.GetQuotAssetVolume(),
			kline.GetTakerBuyBaseAssetVolume(),
			kline.GetTakerBuyQuotAssetVolume(),

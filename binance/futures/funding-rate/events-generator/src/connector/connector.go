...
func (c *Connector) Run() {
	...
	for row := range c.consumer.DataStream {
		...
		funding := local_consumer.Funding{}
		(*row).ScanStruct(&funding)

		events, err := ExtractEvents(funding)
		if err != nil {
			c.logger.Error("failed to extract funding event", slog.String("err", err.Error()))
		}
